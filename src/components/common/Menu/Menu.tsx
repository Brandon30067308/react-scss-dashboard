...
const Menu: FC<MenuProps> = ({ menuGroups, alt = false, showCaveat = true, classes, def, style }) => {
    ...
    <div
      ...
      onKeyDown={e => {
        ...
        switch (e.code) {
          ...
          case 'ArrowDown': {
            const isDivide = (menuGroups[selected[0]].children.length - 1) >= (selected[1] + 1) &&
              menuGroups[selected[0]].children[selected[1] + 1] === 'menu-divide';
            const hasReachedEnd =
              (menuGroups[selected[0]].children.length - 1) === selected[1] ? true :
                !isDivide ? false : (menuGroups[selected[0]].children.length - 1) === selected[1] + 1;
            const cellIndex = menuGroups[selected[0]].children[(selected[1] + 1)] === 'menu-divide' ?
              selected[1] + 2 : selected[1] + 1;
            setSelected(prev => {
              const indices: [number, number] = hasReachedEnd ?
                (menuGroups.length - 1 >= prev[0] + 1) ?
                  [prev[0] + 1, menuGroups[prev[0] + 1].children[0] === 'menu-divide' ? 1 : 0] :
                  prev :
                [prev[0], cellIndex];
              return indices;
            });
          ...
          case 'ArrowUp': {
            const isDivide = 0 <= (selected[1] - 1) &&
              menuGroups[selected[0]].children[selected[1] - 1] === 'menu-divide';
            let hasReachedStart = selected[1] === 0 ? true :
              !isDivide ? false : (selected[1] + 1 === 0);
            const cellIndex = menuGroups[selected[0]].children[selected[1] - 1] === 'menu-divide' ?
              selected[1] - 2 : selected[1] - 1;
            const prevGroupLastIndex = menuGroups[selected[0] - 1] &&
              menuGroups[selected[0] - 1].children.length - 1;
            setSelected(prev => {
              const indices: [number, number] = hasReachedStart ?
                (0 <= prev[0] - 1) ?
                  [prev[0] - 1, menuGroups[prev[0] - 1].children[prevGroupLastIndex] === 'menu-divide' ?
                    prevGroupLastIndex - 1 : prevGroupLastIndex] :
                  prev :
                [prev[0], cellIndex];
              return indices;
            });
      ...
      {alt ?
        ...
        >
          <span className="font-md ">{def || menuGroups[selected[0]].children[selected[1]]}</span>
          {showCaveat &&
            <FontAwesomeIcon
      ...
      }
      {
        ...
        >
          {
            menuGroups && menuGroups.map((group, gi) => (
              <ul
                key={`menu-group-${gi}`}
              ...
              >
                {
                  group.header &&
                  (<span className={`${MenuStyles.groupHeader}
                ...
                }
                {
                  group.children.map((c, i) => {
                    ...
                    return c === 'menu-divide' ?
                      ...
                      <li
                        key={`menu-item-${i}`}
                        className={`${MenuStyles.item} ${isSelected &&
                          MenuStyles.selectedItem} ${alt && 'w-100 text-align-left'} 
